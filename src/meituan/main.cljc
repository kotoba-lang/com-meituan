(ns meituan.main
  "Kotodama WASM entrypoint for the Meituan clean-room actor (L4) — Clojure port.
  ...
  facts (`meituan.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "meituan")
(def tier "L4")
...
(def entity-specs
  [{:entity "Product"    :plural "products"    :id-prefix "meituan_pro"
   ...
   {:entity "Customer"   :plural "customers"   :id-prefix "meituan_cus"
    :fields [:email :firstName :lastName :phone] :required [:email :firstName]
    :coerce {}                       :refs {}}
   {:entity "Collection" :plural "collections" :id-prefix "meituan_col"
...
(defn emit-facts
  "EAVT facts for one record: {\"meituan.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "meituan-compat" :tier tier :entities entities} 200])
...
(defn start! [] :meituan-compat/ready)
